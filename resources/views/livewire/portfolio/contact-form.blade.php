<div class="rounded-[1.75rem] border border-[#cfdae6] bg-white/90 p-5 shadow-[0_24px_70px_-42px_rgba(11,31,51,0.45)] transition-colors dark:border-white/12 dark:bg-[#050e1a] dark:shadow-[0_28px_75px_-38px_rgba(0,0,0,0.95)] sm:p-7 lg:p-8">
    ...
    <div aria-live="polite" aria-atomic="true">
        @if ($successMessage)
            <div id="contact-form-success" class="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm leading-6 text-emerald-900 dark:border-emerald-400/25 dark:bg-emerald-400/10 dark:text-emerald-200" role="status">
                {{ $successMessage }}
            </div>
        @endif
    </div>
    ...
    <div aria-live="assertive" aria-atomic="true">
        @if ($errorMessage)
            <div id="contact-form-error" class="mb-6 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm leading-6 text-rose-900 dark:border-rose-400/25 dark:bg-rose-400/10 dark:text-rose-200" role="alert">
                {{ $errorMessage }}
            </div>
        @endif
    </div>
    ...
    <form wire:submit="submit" class="relative space-y-5" aria-label="Formulario de contacto">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <div>
                <label for="contact-name" class="mb-2 block text-sm font-medium text-[#0b1f33] dark:text-slate-200">
                    Nombre <span class="text-sky-700 dark:text-sky-300" aria-hidden="true">*</span>
                </label>
                <input
                    id="contact-name"
                    type="text"
                ...
                >
                @error('name')
                    <p id="contact-name-error" class="mt-2 text-sm text-rose-700 dark:text-rose-300" role="alert">{{ $message }}</p>
                @enderror
            </div>

            <div>
                <label for="contact-email" class="mb-2 block text-sm font-medium text-[#0b1f33] dark:text-slate-200">
                    Correo <span class="text-sky-700 dark:text-sky-300" aria-hidden="true">*</span>
                </label>
                <input
                    id="contact-email"
                    type="email"
                ...
                >
                @error('email')
                    <p id="contact-email-error" class="mt-2 text-sm text-rose-700 dark:text-rose-300" role="alert">{{ $message }}</p>
                @enderror
            </div>
        </div>

        <div>
            <label for="contact-subject" class="mb-2 block text-sm font-medium text-[#0b1f33] dark:text-slate-200">
                Asunto <span class="text-sky-700 dark:text-sky-300" aria-hidden="true">*</span>
            </label>
            <input
                id="contact-subject"
                type="text"
            ...
            >
            @error('subject')
                <p id="contact-subject-error" class="mt-2 text-sm text-rose-700 dark:text-rose-300" role="alert">{{ $message }}</p>
            @enderror
        </div>

        <div>
            <label for="contact-body" class="mb-2 block text-sm font-medium text-[#0b1f33] dark:text-slate-200">
                Mensaje <span class="text-sky-700 dark:text-sky-300" aria-hidden="true">*</span>
            </label>
            <textarea
                id="contact-body"
        ...
        <div class="flex flex-col gap-3 pt-1 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-xs leading-5 text-slate-500 dark:text-slate-400">Los campos marcados con * son obligatorios.</p>
            <button
                type="submit"
                wire:loading.attr="disabled"
                wire:target="submit"
                class="inline-flex min-h-12 items-center justify-center rounded-full bg-[#0b1f33] px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-600 disabled:cursor-wait disabled:opacity-65 dark:bg-signal-600 dark:text-white dark:hover:bg-signal-500"
            >
