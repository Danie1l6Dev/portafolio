<div class="space-y-6">
    <header class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><flux:heading size="xl" level="1">Mapa de habilidades</flux:heading><flux:text variant="subtle">Mantén legible tu perfil técnico por grupo, dominio y relevancia.</flux:text></div>
    ...
    <div class="grid gap-6 {{ $showForm ? 'xl:grid-cols-[minmax(0,1fr)_24rem]' : '' }}">
            ...
            <div class="overflow-hidden rounded-xl border border-zinc-200/80 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full min-w-[46rem] text-left text-sm">
                        <thead class="bg-zinc-50 text-xs uppercase tracking-wider text-zinc-500 dark:bg-zinc-800/70"><tr><th class="px-4 py-3 font-medium">Habilidad</th><th class="px-4 py-3 font-medium">Grupo</th><th class="px-4 py-3 font-medium">Dominio</th><th class="px-4 py-3 font-medium">Uso</th><th class="px-4 py-3 text-right font-medium">Acciones</th></tr></thead>
                        <tbody class="divide-y divide-zinc-200/70 dark:divide-zinc-700">
                            @forelse ($skills as $skill)
                                <tr wire:key="skill-{{ $skill->id }}" class="hover:bg-zinc-50/70 dark:hover:bg-zinc-800/40">
                                    ...
                                    <td class="px-4 py-3"><div class="flex items-center gap-2" aria-label="Nivel {{ $skill->level }} de 5"><div class="flex gap-1">@for($level = 1; $level <= 5; $level++)<span class="h-1.5 w-4 rounded-full {{ $level <= $skill->level ? 'bg-zinc-900 dark:bg-zinc-100' : 'bg-zinc-200 dark:bg-zinc-700' }}"></span>@endfor</div><span class="text-xs tabular-nums text-zinc-500">{{ $skill->level }}/5</span></div></td>
                            ...
                            @empty
                                <tr><td colspan="5" class="px-6 py-12 text-center"><flux:heading size="lg">Sin coincidencias técnicas</flux:heading><flux:text variant="subtle" class="mt-1">Ajusta los filtros o registra una habilidad.</flux:text></td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        @if ($showForm)
            <aside class="h-fit rounded-xl border border-zinc-200/80 bg-zinc-50/70 p-5 dark:border-zinc-700 dark:bg-zinc-900" aria-label="Formulario de habilidad">
                <div class="mb-5"><flux:heading size="lg">{{ $editingSkillId ? 'Editar habilidad' : 'Nueva habilidad' }}</flux:heading><flux:text variant="subtle" class="text-sm">Define dónde vive y cuánto pesa en tu perfil.</flux:text></div>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4"><flux:input wire:model="level" label="Nivel (1–5)" type="number" min="1" max="5" /><flux:input wire:model="sortOrder" label="Orden" type="number" min="0" /></div>
