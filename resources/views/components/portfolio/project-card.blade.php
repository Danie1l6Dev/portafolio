...
<article {{ $attributes->class('group flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200/90 bg-white shadow-[0_18px_45px_-34px_rgba(15,23,42,0.45)] transition duration-200 hover:-translate-y-1 hover:border-sky-200 hover:shadow-[0_24px_55px_-32px_rgba(2,132,199,0.35)]') }}>
    <a
        href="{{ $detailUrl }}"
        class="relative block aspect-[16/10] overflow-hidden bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-sky-500"
    ...
    <div class="flex flex-1 flex-col p-6 sm:p-7">
        ...
        <h2 class="text-xl font-semibold tracking-[-0.025em] text-slate-950 sm:text-2xl">
            <a href="{{ $detailUrl }}" class="transition-colors hover:text-sky-700 focus-visible:rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500">
                {{ $project->title }}
            </a>
        </h2>

        <p class="mt-3 flex-1 text-sm leading-7 text-slate-600 sm:text-[0.95rem]">
            {{ $project->summary }}
        </p>
        ...
        @if ($project->skills->isNotEmpty())
            <ul class="mt-5 flex flex-wrap gap-2" aria-label="Tecnologías utilizadas">
                @foreach ($project->skills->take(4) as $skill)
                    <li class="rounded-lg bg-slate-100 px-2.5 py-1.5 font-mono text-[0.68rem] font-medium text-slate-600">
                        {{ $skill->name }}
                    </li>
                @endforeach

                @if ($project->skills->count() > 4)
                    <li class="rounded-lg bg-slate-50 px-2.5 py-1.5 font-mono text-[0.68rem] font-medium text-slate-400">
        ...
        <div class="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5">
            <a
                href="{{ $detailUrl }}"
                class="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-slate-900 transition-colors hover:text-sky-700 focus-visible:rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
            >
                Ver caso
            ...
            <div class="flex items-center gap-2">
                @if ($project->demo_url)
                    <a
                        ...
                        class="inline-flex min-h-10 items-center rounded-xl px-3 text-xs font-semibold text-slate-500 transition-colors hover:bg-sky-50 hover:text-sky-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
